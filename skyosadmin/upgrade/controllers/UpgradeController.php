...
class UpgradeController extends PolicyController {
	...
	public function edit($editArr){
		...
		$Arr = array(
				        ...
				        'desc'=>$editArr['desc'],
				        'core_style'=>$editArr['core_style'],
						'valid_mac'=>$editArr['valid_mac']
		);
		//返回1表示成功，返回0表示失败
		$result = UpgradeModel::updateSysUpgrade($Arr);
		if($result&&isset($editArr['sendEmail'])&&$editArr['sendEmail']){
			...
			if($mail){
				...
				$mail->AddAddress('Huangyixiang@skyworth.com');
				$mail->AddAddress('LiuJinPeng@skyworth.com');
				$mail->AddAddress('yangyan03@skyworth.com');
				$mail->AddAddress('ChenYongBin@skyworth.com');
				$mail->AddAddress('baolifang@skyworth.com');
				$mail->SetFrom('skysrt@163.com');
				$mail->AddReplyTo('skysrt@163.com');
				$mail->Subject = '升级包开放MAC段通知';
				$mail->Body = "在服务器域名：".HostName." ,区域编码为 ".$editArr['area']." 的服务器，于".$t." ，将机型为： ".$editArr['core_style']." ，机芯为：".$editArr['core_chip']." ,初始版本：".$editArr['init_version']." ，最终版本：".$editArr['final_version']." 的升级包MAC段开放为：".$editArr['mac_start']."--".$editArr['mac_end']."\r\n备注：区域编码对应表为1111(tvos公网服务器)，1113(sky国庆量产服务器)，……";
				$mail->Send();
			}
		}
		return $result;
